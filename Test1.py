...
expName = 'Mem_Test1'  # from the Builder filename that created this script
expInfo = {'participant': ''}
...
filename = _thisDir + os.sep + u'data_test1/%s_%s_%s' %(expInfo['participant'], expName, expInfo['date'])

# An ExperimentHandler isn't essential but helps with data saving
thisExp = data.ExperimentHandler(name=expName, version='',
    extraInfo=expInfo, runtimeInfo=None,
    originPath='/Users/fernanda/Library/CloudStorage/Box-Box/Leal Lab/Behavioral Tasks/In-Person Tasks/Memorability/memorability/Test1.py',
...
trials = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('example_test1.xlsx', selection='0:5'),
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        # *example_key* updates
        if example_key.status == NOT_STARTED and t >= .5-frameTolerance:
            # keep track of start time/frame for later
...
Task = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('StimData_test1.xlsx', selection='0:161'),
    seed=None, name='Task')
...
for thisTask in Task:
    ...
    # keep track of which components have finished
    testComponents = [ISI, Stimuli, Key_Response, text_test]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if Key_Response.status == STARTED and not waitOnFlip:
            theseKeys = Key_Response.getKeys(keyList=['V', 'N', 'v', 'n'], waitRelease=False)
        ...
        # *text_test* updates
        if text_test.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if text_test.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_test.tStartRefresh + 3.5-frameTolerance:
                # keep track of stop time/frame for later
                text_test.tStop = t  # not accounting for scr refresh
                text_test.frameNStop = frameN  # exact frame index
                win.timeOnFlip(text_test, 'tStopRefresh')  # time at next scr refresh
    ...
    for thisComponent in testComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    Task.addData('ISI.started', ISI.tStart)
    Task.addData('ISI.stopped', ISI.tStop)
    # check responses
    if Key_Response.keys in ['', [], None]:  # No response was made
        Key_Response.keys = None
    Task.addData('Key_Response.keys',Key_Response.keys)
    if Key_Response.keys != None:  # we had a response
        Task.addData('Key_Response.rt', Key_Response.rt)
    Task.addData('Key_Response.started', Key_Response.tStartRefresh)
    Task.addData('Key_Response.stopped', Key_Response.tStopRefresh)
    thisExp.nextEntry()
